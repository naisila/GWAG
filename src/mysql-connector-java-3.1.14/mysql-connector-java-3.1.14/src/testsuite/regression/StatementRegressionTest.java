...
public class StatementRegressionTest extends BaseTestCase {
	...
	public void testBug20687() throws Exception {
		if (!isRunningOnJdk131() && versionMeetsMinimum(4, 1)) {
			createTable("testBug20687", "(field1 int)");
			Connection poolingConn = null;
			
			Properties props = new Properties();
			props.setProperty("cachePrepStmts", "true");
			props.setProperty("prepStmtCacheSqlLimit", "5000");
			
			StringBuffer queryBuf = new StringBuffer("SELECT field1 FROM testBug20687 /*");
			...
			try {
				...
				PreparedStatement pstmt1 = poolingConn.prepareStatement(query);
				pstmt1.executeQuery();
				pstmt1.close();
				
				PreparedStatement pstmt2 = poolingConn.prepareStatement(query);
				pstmt2.executeQuery();
